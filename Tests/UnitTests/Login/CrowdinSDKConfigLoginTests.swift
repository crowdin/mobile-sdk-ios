...
class CrowdinSDKConfigLoginTests: XCTestCase {
    ...
    override func setUp() {
        ...
        config = CrowdinSDKConfig.config(organizationName: "organizationName").with(loginConfig: loginConfig)
    }
    ...
    func testChangeConfigAfterSetup() {
        ...
        XCTAssert(config.loginConfig?.organizationName == "organizationName")
    }
}
