---
description: Preview translations made in the Crowdin Editor directly in your iOS app in real time. Learn how to set up and authorize the Real-Time Preview feature.
---
...
If you installed the SDK via Swift Package Manager, no additional setup is needed - the Real-Time Preview functionality is already included in the `CrowdinSDK` package.
:::
...
To enable OAuth-based authorization, add the following configuration to your `AppDelegate` class in the `application` method:
