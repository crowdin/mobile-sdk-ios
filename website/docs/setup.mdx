---
description: Configure the Crowdin iOS SDK in your app - enable Over-The-Air Content Delivery and explore config options and additional features.
---
...
**Distribution** is a CDN vault that mirrors the translated content of your project and is required for integration with the iOS app.

- [Creating a distribution in crowdin.com](https://support.crowdin.com/cdn-distributions/)
...
1. Add these keys to *Info.plist*:
   ...
   - `CrowdinSourceLanguage` (String) - Source language code in [ISO 639-1](https://www.loc.gov/standards/iso639-2/php/English_list.php) format
...
| `sourceLanguage`   | Source language code ([ISO 639-1](https://www.loc.gov/standards/iso639-2/php/English_list.php))                              | `sourceLanguage: "en"` |
...
By default, the SDK searches for new translations once per application load, but not more often than 15 minutes. You can update translations in the application every defined interval. To enable this feature, add the `CrowdinSDK/IntervalUpdate` pod to your `Podfile`:
...
2.  Configure in SDK:
    ...
    Where `interval` defines the translations update time interval in seconds. The minimum allowed interval is 15 minutes (900 seconds).

### Change locale programmatically

By default, the SDK relies on the device locale. To change the SDK target language on the fly regardless of the device locale, use the `CrowdinSDK.currentLocalization` property to get or set the current localization language code ([ISO 639-1](https://www.loc.gov/standards/iso639-2/php/English_list.php)) used by the Crowdin SDK. When you set this property, the SDK triggers a localization download if needed:
...
View the [SwiftUI Localization Guide](/guides/swift-ui) for more information.
