...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="CrowdinSDK",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC10CrowdinSDK10CrowdinSDK")
@interface CrowdinSDK : NSObject
SWIFT_CLASS_PROPERTY(@property (nonatomic, class) enum Mode mode;)
+ (enum Mode)mode SWIFT_WARN_UNUSED_RESULT;
+ (void)setMode:(enum Mode)newValue;
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, copy) NSString * _Nullable currentLocalization;)
+ (NSString * _Nullable)currentLocalization SWIFT_WARN_UNUSED_RESULT;
+ (void)setCurrentLocalization:(NSString * _Nullable)newValue;
+ (void)start;
+ (void)startWith:(id <LocalizationProvider> _Nullable)provider;
...
@interface UIViewController (SWIFT_EXTENSION(CrowdinSDK))
